...
def extract_features(image_names):
    """
    extract features in all images

    params:
        image_names: list of all image filenames
    """
...
def main():
    ...
    frame_dirs = sorted(glob.glob(os.path.join(imgdir, "*/")))
    img_names = []
    for frame_dir in tqdm(frame_dirs):
        with open(os.path.join(frame_dir, 'camera.csv')) as f:
            type = f.read().split('_')[-2]
            if type == 'f':
                img_names.append(os.path.join(frame_dir, "raw_image.jpg"))
    # img_names = sorted(glob.glob(os.path.join(imgdir, '*.' + "jpg")))
    ...
    pcd.points = o3d.utility.Vector3dVector(structure)
    o3d.io.write_point_cloud("structure.ply", pcd)
